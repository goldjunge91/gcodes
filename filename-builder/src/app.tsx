...
const getCategoryColor = (category: string): string => {
  return CAT_COLORS[category as Category] ?? "#94a3b8";
};

function getPreview(template: string): string {
  let preview = template;
  // Handle {if ...}{...}{else}{...}{endif} - simple mock
  preview = preview.replace(/\{if\s+plate_name!=""\}(.*?)\{else\}(.*?)\{endif\}/gs, (_match: string, _ifValue: string, elseValue: string) => elseValue);
  preview = preview.replace(/\{if\s+.*?\}(.*?)\{else\}(.*?)\{endif\}/gs, (_match: string, ifValue: string) => ifValue); // General mock
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-slate-200 font-sans selection:bg-orange-500/30">
      ...
      <main className="mx-auto px-8 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {["Alle", ...CATEGORIES].map(cat => {
                  const color = cat === "Alle" ? "#ffffff" : getCategoryColor(cat);
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span 
                      ...
                      style={{ color: getCategoryColor(v.category) }}
                    >
                      {v.icon}
                    </span>
                    <span 
